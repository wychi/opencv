...
class CppHeaderParser(object):
    ...
    def parse_class_decl(self, decl_str):
        ...
        l = self.batch_replace(l, [("CV_EXPORTS_JS", ""), ("CV_EXPORTS_W", ""), ("CV_EXPORTS", ""), ("public virtual ", " "), ("public ", " "), ("::", ".")]).strip()
        ll = re.split(r'\s*[,:]?\s*', l)
            ...
            if stmt.startswith("class") or stmt.startswith("struct"):
                ...
                if stmt.strip() != stmt_type:
                    try:
                        ...
                        exit(1)
                    decl = []
                    if ("CV_EXPORTS_JS" in stmt) or ("CV_EXPORTS_AS" in stmt) or (not self.wrap_mode):# and ("CV_EXPORTS" in stmt)):
                        decl = [stmt_type + " " + self.get_dotted_name(classname), "", modlist, []]
                        if bases:
                            decl[1] = ": " + ", ".join([self.get_dotted_name(b).replace(".","::") for b in bases])
                    return stmt_type, classname, True, decl
