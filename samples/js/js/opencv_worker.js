...
WorkerCommandDispatcher = {
    ...
    Module._free(binSize);
  	...
  	return [
  	  ...
  	  { buffer: pout.data.buffer,
				...
				histogramBuffer: histImageData.data.buffer,
				histogramBins: bins,
				histogramLength: histImageData.height,
			},
			[pout.data.buffer]
		];
	},
  ...
  receiveMessage: function (settings) {
		...
		settings.forEach(function (v) {
			...
			if (!!algorithm) {
				var tupple = algorithm(setting, imageData);

				imageData = tupple[0];
				aMessage[setting.id] = tupple[1];
				transferList.concat(tupple[2]);

				console.assert(tupple.length === 3);
				console.assert(!!tupple[0] && !!tupple[1] && !!tupple[2]);
			}
		});
